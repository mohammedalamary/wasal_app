...
class AccountParentController extends GetxController{
  ...
  void onInit() {
    ...
    email = TextEditingController(text: sharedPreferences.getString(Global.parentEmail)??"");
    userName = TextEditingController(text: sharedPreferences.getString(Global.parentName)??"");
    super.onInit();
  ...
  updateAccount()async{
    ...
    if( StatusRequest.success==statusRequest){
      if(response["status"]=="1"){
        sharedPreferences.setString(Global.parentName,userName.text.toString());
        sharedPreferences.setString(Global.parentEmail,email.text.toString());
